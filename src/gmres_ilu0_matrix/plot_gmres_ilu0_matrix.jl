...
# Create plots
function plot_gmres_ilu0_matrix(benchstep::String)

	# Runtimes

	# Get results
	x = get_matrices()
	y_ser = get_runtimes("ser", benchstep)[1]
	y_par = get_runtimes("par", benchstep)
	...
	# Save plot
	savefig(string("gmres_ilu0_matrix_", benchstep, "_speedups.png"))
	savefig(string("gmres_ilu0_matrix_", benchstep, "_speedups.tex"))

end

function plot_gmres_ilu0_matrix()
	plot_gmres_ilu0_matrix("solver")
	plot_gmres_ilu0_matrix("precon")
end
